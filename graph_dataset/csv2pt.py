...
def get_three_basic_info(dataset):
    ...
    _dim_features = dataset.data[0].x.size(1)
...
class Graph2feats():
    ...
    def df2files(self):
        ...
        fp_graph_indicator = open(f"{raw_dir}/{self.name}_graph_indicator.txt", "a")
        fp_graph_labels = open(f"{raw_dir}/{self.name}_graph_labels.txt", "a")
        fp_node_labels = open(f"{raw_dir}/{self.name}_node_labels.txt", "a")
        fp_node_labels = open(f"{raw_dir}/{self.name}_node_labels.txt", "a")
        fp_node_attrs = open(f"{raw_dir}/{self.name}_node_attributes.txt", "a")
        fp_edge_attrs = open(f"{raw_dir}/{self.name}_edge_attributes.txt", "a")
        # r defer close
